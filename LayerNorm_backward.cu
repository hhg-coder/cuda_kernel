...
//dinp 输入梯度（输出参数），形状 [B, T, C]，反向传播中损失对输入 inp的梯度
//dweight 权重梯度（输出参数），形状 [C]，损失对缩放参数 weight的梯度
//...
//dout 上层梯度（输入参数），形状 [B, T, C]，损失对 LayerNorm 输出的梯度
//weight 缩放参数（输入参数），形状 [C]，前向传播中的仿射变换系数 γ
//...
void layernorm_backward_cpu(float *dinp, float *dweight, float* dbias,
    ...
    for (int b = 0; b < B; b++) {
        for (int t = 0; t < T; t++) {
            ...
            for (int i = 0; i < C; i++) {
                float norm_bti = (inp_bt[i] - mean_bt) * rstd_bt;// 归一化值：x̂ = (x - μ) / σ
                float dnorm_i = weight[i] * dout_bt[i];          // w * ∂L/∂y
                dnorm_mean += dnorm_i;// 累加  w * ∂L/∂y 的均值
                dnorm_norm_mean += dnorm_i * norm_bti;// 累加 (w * ∂L/∂y) * x̂ 的均值
            }
            dnorm_mean = dnorm_mean / C;// 计算均值：E[w * ∂L/∂y]
            dnorm_norm_mean = dnorm_norm_mean / C;// 计算均值：E[(w * ∂L/∂y) * x̂]
            ...
            for (int i = 0; i < C; i++) {
                ...
                dweight[i] += norm_bti * dout_bt[i];// ∂L/∂w = x̂ * ∂L/∂y
                // gradient contribution to input
                // 输入梯度：∂L/∂x_i = (w_i * ∂L/∂y_i - E[w·∂L/∂y] - x̂_i · E[(w·∂L/∂y)·x̂]) / σ
                float dval = 0.0f;
                dval += dnorm_i; // term 1
