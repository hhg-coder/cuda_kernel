...
// ----------------------------------------------------------------------------
// CPU code reference
// preatt 原始注意力分数：Softmax前的Q·K^T得分矩阵
//att 归一化注意力权重：Softmax后的概率分布
// NH 注意力头数：多头注意力机制的头数
...
//flash attention
//可参考  https://blog.csdn.net/v_JULY_v/article/details/133619540
//可参考  https://zhuanlan.zhihu.com/p/638468472
void attention_forward2(float* out,
    ...
    //过kernel融合的方式，将多个操作融合为一个操作，利用高速的SRAM进行计算，可以减少读写HBM的次数，从而有效减少内存受限操作的运行时间。
    //内存受限 --> 减少HBM读写次数 --> kernel融合 --> 满足SRAM的内存大小 --> 分块计算，因此分块大小block_size不能太大，否则会导致OOM
    ...
    int num_blocks = ceil_div(total_threads, block_size);
    //避免在 flash attention 中进行复杂的索引、reshape 及 permute 操作，首先使用一个kernel 完成这些操作
